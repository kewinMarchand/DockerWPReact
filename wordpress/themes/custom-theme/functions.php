...
function createAppTag() {
    $description = esc_attr( get_bloginfo( 'description' ) );
    $menu = htmlspecialchars(json_encode( wp_get_nav_menu_items( wp_get_nav_menu_object( get_nav_menu_locations()[ 'main-menu' ] )->term_id, array( 'order' => 'DESC' ) ) ));
    $name = esc_attr( get_bloginfo( 'name' ) );
    $url = esc_attr( get_bloginfo( 'wpurl' ) );
    $user = htmlspecialchars(json_encode(wp_get_current_user()), ENT_QUOTES, 'UTF-8');

    $current_page = get_queried_object();

    $content  = htmlspecialchars(json_encode(apply_filters( 'the_content', $current_page->post_content )), ENT_QUOTES, 'UTF-8');

    $posts = htmlspecialchars(json_encode(get_posts()));
    $page = htmlspecialchars(json_encode(get_queried_object()));

    $thumbs = array();
    foreach (get_posts() as $post) {
        $thumbs[$post->ID] = json_encode( get_the_post_thumbnail_url( $post ) );
    }

    echo '<div id="app"
        data-wp_page="' . $page . '"
        data-wp_content="' . $content . '"
        data-wp_description="' . $description . '"
        data-wp_menu="' . $menu . '"
        data-wp_name="' . $name . '"
        data-wp_posts="' . $posts . '"
        data-wp_url="' . $url . '"
        data-wp_user="' . $user . '"
        data-wp_thumbs="' . htmlspecialchars(json_encode($thumbs)) . '"
    ></div>';
}
